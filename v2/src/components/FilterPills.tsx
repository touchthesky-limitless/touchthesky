...
export default function FilterPills({
	...
	const hasFilters =
		search || activeBanks.length > 0 || activeAlliances.length > 0;
	if (!hasFilters) return null;

	// The specific base class you liked
	const base =
		"px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-tighter transition-all cursor-pointer border flex items-center gap-1.5 shadow-sm";
	...
	return (
		<div className="flex flex-wrap gap-2 mb-4 animate-in fade-in slide-in-from-left-2 duration-300">
			...
			{activeAlliances.map((alliance) => (
				<button
					key={alliance}
					onClick={() => onRemoveAlliance(alliance)}
