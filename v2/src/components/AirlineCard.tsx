...
export default function AirlineCard({ airline, ALLIANCE_LOGOS, isGridView, banks }: any) {
  const [imgError, setImgError] = useState(false);

  const airlineLogoUrl = airline.logo 
    ? (airline.logo.startsWith('http') ? airline.logo : `/logos/${airline.logo}`)
    : `https://www.google.com/s2/favicons?sz=128&domain=${airline.domain}`;

  const getBankLogo = (bankId: string, banksList: any[]) => {
    const bank = banksList?.find(b => b.id.toLowerCase() === bankId.toLowerCase());
        ...
        <div className="relative flex-shrink-0">
          <div className="w-12 h-12 rounded-2xl bg-white p-2 flex items-center justify-center shadow-md border border-slate-100 dark:border-transparent">
            {imgError ? (
              ...
              <img src={airlineLogoUrl} onError={() => setImgError(true)} className="w-full h-full object-contain" alt="" />
            )}
          </div>
        </div>
        ...
        {airline.partners?.map((p: any, i: number) => (
          ...
          >
            <div className="w-7 h-7 rounded-lg bg-white p-1 flex items-center justify-center shadow-sm flex-shrink-0 border border-slate-100">
              <img src={getBankLogo(p.bank, banks)} className="w-full h-full object-contain" alt="" />
            </div>
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-[11px] font-bold leading-none">{p.ratio}</span>
                {p.bonus && (
