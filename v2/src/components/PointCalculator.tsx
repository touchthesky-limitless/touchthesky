...
export default function PointCalculator({ setView }: PointCalculatorProps) {
	...
	const handleCopy = () => {
		...
		const fTarget = (
			parseInt(pointsNeeded.replace(/,/g, "")) || 0
		).toLocaleString();
	...
	const presets = useMemo(() => generatePresetsFromData(AIRLINES, BANKS), []);
					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="text-xs text-slate-500 dark:text-slate-400">
								Current promos can save you thousands of points.
							</p>
						</div>
					</div>
					...
					<div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-2">
						{presets.map((bank) => {
							...
							return (
                                ...
                                `}
								>
									<span className="text-[7px] font-black uppercase mb-1">
										{bank.label}
									</span>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
				<div className="bg-blue-600 p-6 rounded-2xl text-white shadow-lg shadow-blue-500/20 relative group overflow-hidden">
					...
					<div className="pointer-events-none">
						...
						{/* NEW ALERT LOGIC */}
						{pointsToTransfer !== result.exactRequired &&
							pointsToTransfer > 0 && (
				...
				<div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
					...
					<p className="text-3xl font-black text-slate-900 dark:text-white">
						{result.totalPointsReceived.toLocaleString()}
					</p>
				</div>
				...
				<div className="mt-8 space-y-4 animate-in slide-in-from-bottom-4 duration-500">
					...
					<div className="grid grid-cols-1 gap-3">
						{COMPARISON_PARTNERS.map((partner) => {
							...
							const inputAmount =
								parseFloat(pointsNeeded.replace(/,/g, "")) || 0;
					...
					<p className="text-[10px] text-center text-slate-400 italic">
						...
						{mode === "need" ? "Points Needed" : "Points I Have"}" input
					</p>
				</div>
			)}
		</div>
