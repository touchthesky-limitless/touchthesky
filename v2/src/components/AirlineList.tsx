...
export default function AirlineList({
	...
	const { paginatedData, hasMore, remaining, showMore, showAll } =
		usePagination(airlines, initialCount);
		...
		<div className="space-y-6">
			...
			{hasMore && (
				<div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8 mb-12">
					<button
						onClick={showMore}
						className="cursor-pointer disabled:cursor-not-allowed w-full sm:w-auto px-8 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 active:scale-95 transition-all shadow-sm"
					>
						Show More ({remaining})
					</button>
					<button
						onClick={showAll}
						className="cursor-pointer disabled:cursor-not-allowed w-full sm:w-auto px-8 py-3 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-all"
					>
						Show All
					</button>
				</div>
